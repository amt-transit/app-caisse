            ...
            <div id="app" v-cloak>
                <div class="dashboard-container sal-page" v-if="user && !authLoading">
                    ...
                    <div v-if="currentSalaireView === 'dashboard'">
                        ...
                        <div v-if="isSuperAdmin" style="margin-bottom:20px; display:flex; gap:10px; flex-wrap:wrap;">
                            <button class="btn btn-success" @click="showFundModal = true">💰 Allouer un budget (mois)</button>
                        </div>
                        <div class="card">
                    ...
                    <!-- MODALS -->
                    <div v-if="showFundModal" class="modal active" style="display:flex;"><div class="modal-content" style="max-width:460px;"><span class="close-modal" @click="showFundModal = false">&times;</span><h2>💰 Allouer un budget (Dotation)</h2><p style="color:#64748b; font-size:13px; margin-top:0;">Définissez le fonds disponible pour payer les salaires d'un mois.</p><div class="form-group"><label>Mois concerné</label><input type="month" v-model="selectedBudgetMonth"></div><div class="form-group"><label>Montant du budget (CFA)</label><input type="number" v-model="newFund.amount" placeholder="Ex : 2000000"></div><div class="form-group"><label>Note (optionnel)</label><input type="text" v-model="newFund.note" placeholder="Ex : Dotation caisse"></div><button class="btn btn-success" style="width:100%; margin-top:15px;" @click="saveSalaryFund" :disabled="actionLoading">Enregistrer le budget</button><div v-if="salaryFunds.filter(f => f.targetMonth === selectedBudgetMonth).length" style="margin-top:18px;"><h3 style="font-size:14px; margin-bottom:8px;">Budgets alloués pour {{ selectedBudgetMonth }}</h3><table class="table" style="font-size:13px; margin:0;"><tbody><tr v-for="f in salaryFunds.filter(f => f.targetMonth === selectedBudgetMonth)" :key="f.id"><td style="font-weight:bold;">{{ formatMoney(f.amount) }}</td><td style="color:#64748b;">{{ f.note }}</td><td style="text-align:right;"><button class="btn btn-small btn-danger" @click="deleteSalaryFund(f.id)">🗑️</button></td></tr></tbody></table></div></div></div>
