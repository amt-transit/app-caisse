...
export const ScanDechargementView = {
    ...
    render(app, container) {
        ...
        window.app.views.scanDechargement = this;
        this.recentScans = [];
        ...
        const html = `
            <style>
                ...
                .container-select-input:focus { border-color: #3b82f6; background: white; box-shadow: 0 0 0 3px rgba(59,130,246,0.1); }
            ...
            <div class="sw-page" id="sw-page">
                <div class="sm__header">
                    <div class="sm__header-inner">
                        <div class="sm__header-info">
                            ...
                            <div>
                                <h1 class="sm__header-title">Déchargement Conteneur</h1>
                                <p class="sm__header-desc">Scannez les colis à leur arrivée à Abidjan pour démarrer le stockage.</p>
                            </div>
                        </div>
                        <div class="sm__header-actions">
                            <button class="sm__btn-sound" id="btn-sound-dec" type="button" title="Activer/Désactiver le son" onclick="window.app.views.scanDechargement.toggleSound()">🔊</button>
                            <button class="sm__btn-clear" type="button" title="Effacer la session" onclick="window.app.views.scanDechargement.clearSession()">🗑️</button>
                        </div>
                    </div>
                </div>
                
                <div class="sm__kpi-row">
                    <div class="sm__kpi sm__kpi--blue"><div class="sm__kpi-val" id="dec-kpi-total">0</div><div class="sm__kpi-lbl">Total</div></div>
                    <div class="sm__kpi sm__kpi--green"><div class="sm__kpi-val" id="dec-kpi-success">0</div><div class="sm__kpi-lbl">Succès</div></div>
                    <div class="sm__kpi sm__kpi--orange"><div class="sm__kpi-val" id="dec-kpi-duplicate">0</div><div class="sm__kpi-lbl">Déjà traité</div></div>
                    <div class="sm__kpi sm__kpi--red"><div class="sm__kpi-val" id="dec-kpi-error">0</div><div class="sm__kpi-lbl">Erreurs</div></div>
                </div>

                <div class="container-selector">
                    <label>Sélectionnez le conteneur en cours de déchargement :</label>
                    <select id="sd-target-container" class="container-select-input">
                ...
                <div class="manual-row">
                    <input type="text" class="manual-input" id="sw-manual-ref" placeholder="Saisir la référence manuellement..." onkeydown="if(event.key==='Enter') window.app.views.scanDechargement.processManualScan()">
                    <button class="btn-search" onclick="window.app.views.scanDechargement.processManualScan()">Décharger</button>
                </div>

                <div class="recent-scans">
                    <div class="recent-scans-header">
                        <span>📋 Colis déchargés dans la session</span>
                        <span class="recent-count" id="sc-count">0</span>
                    </div>
                    <div id="sw-recent-list">
                        <div style="padding: 30px; text-align: center; color: #94a3b8; font-size: 14px;">Aucun colis déchargé pour le moment.</div>
                    </div>
                </div>
            </div>
        `;
        ...
        // Déverrouillage audio mobile : le son ne peut démarrer qu'après un
        // 1er geste utilisateur (touchstart/click).
    ...
    // Retour combiné son + flash vert du cadre + vibration (comme au départ).
    ensureAudio() {
        try {
    ...
    toggleSound() {
        ...
        const btn = document.getElementById('btn-sound-dec');
    ...
    clearSession() {
        if (confirm("Voulez-vous vraiment effacer les données de scan de cette session ?")) {
            ...
            if (list) list.innerHTML = '<div style="padding: 30px; text-align: center; color: #94a3b8; font-size: 14px;">Aucun colis déchargé pour le moment.</div>';
        }
    },

    updateKPIs() {
        document.getElementById('dec-kpi-total').textContent = this.stats.total;
        document.getElementById('dec-kpi-success').textContent = this.stats.success;
        document.getElementById('dec-kpi-duplicate').textContent = this.stats.duplicate;
        document.getElementById('dec-kpi-error').textContent = this.stats.error;
    },
    ...
    async loadContainers() {
        try {
            ...
            snap.forEach(doc => {
                ...
                opt.value = data.number || doc.id;
                let statusStr = '';
                if (data.status === 'EN_TRANSIT') statusStr = ' (En Mer)';
                if (data.status === 'ARRIVE') statusStr = ' (Arrivé)';
                opt.textContent = (data.number || doc.id) + statusStr;
    ...
    async processScan(text) {
        const targetCont = document.getElementById('sd-target-container')?.value;
        if (!targetCont) {
