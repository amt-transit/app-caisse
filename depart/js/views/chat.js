...
export const ChatView = {
    ...
    async loadSuggestionsData() {
        try {
            ...
            const livSnap = await getDocs(query(collection(db, getCollectionName("livraisons")), where("agency", "==", activeAgency), orderBy("dateAjout", "desc"), limit(2000)));
    ...
    loadMessages() {
        ...
        const activeAgency = sessionStorage.getItem('currentActiveAgency') || 'paris';
        const q = query(collection(db, getCollectionName("internal_chat")), where("agency", "==", activeAgency), orderBy("date", "desc"), limit(100));
