...
export const FactureAerienView = {
    ...
    render(app) {
        ...
        window.app.views.factureAerien = this;

        // Page dédiée à la facturation AÉRIEN au départ de Paris.
        // Le routage des collections (getCollectionName) dépend du mode d'envoi :
        // on exige le mode Aérien pour écrire dans les bonnes tables (_aerien).
        const _mode = sessionStorage.getItem('shippingMode') || 'maritime';
        if (_mode !== 'aerien') {
            document.getElementById('contentContainer').innerHTML = `
                <div style="max-width:640px; margin:40px auto; background:#fff; border:1px solid #e2e8f0; border-radius:16px; padding:30px; text-align:center; box-shadow:0 4px 6px -1px rgba(0,0,0,0.05);">
                    <div style="font-size:42px;">✈️</div>
                    <h2 style="color:#0f172a; margin:10px 0 6px;">Facture Aérien — Paris</h2>
                    <p style="color:#64748b; margin:0 0 18px;">Cette page fonctionne en <b>mode Aérien</b>. Activez le mode Aérien (bouton du mode d'envoi, en haut) puis revenez ici.</p>
                </div>`;
            return;
        }
        ...
        const html = `
            ...
            <div id="vue-factureaerien-app" style="max-width: 1000px; margin: 0 auto; animation: fadeIn 0.3s ease-in-out;" v-cloak>
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; gap: 15px; margin-bottom: 25px; background: white; padding: 20px; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; flex-wrap: wrap;">
                    <div style="display: flex; align-items: center; gap: 15px;">
                        ...
                        <div>
                            <h2 style="margin: 0; color: #0f172a; font-size: 22px;">✈️ Facture Aérien — Paris</h2>
                            <p style="margin: 0; color: #64748b; font-size: 13px;">Expédition aérienne au départ de Paris</p>
                        </div>
                    </div>
                    ...
                    <div style="background:#fff; width:90%; max-width:460px; border-radius:16px; display:flex; flex-direction:column; max-height:90vh; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);">
                        ...
                        <div style="padding:20px; overflow-y:auto; flex:1; min-height:0;">
                            ...
                            <div class="form-group">
                                ...
                                <div style="position:relative;">
                                    <input id="faClientAdresse" type="text" v-model="clientModal.adresse" placeholder="Adresse complète" autocomplete="off" style="width:100%; padding:10px; border:1px solid #cbd5e1; border-radius:8px; box-sizing:border-box; font-size:14px;">
                                    <ul id="faClientAdresseSuggestions" style="margin:0; padding:0; list-style:none; display:none;"></ul>
                                </div>
                            </div>
                        </div>
...
initVue(globalApp) {
    ...
    this.vueApp = createApp({
        setup() {
            ...
            const form = reactive({
                ...
                totalCfa: 0           // Total facturé (CFA), modifiable
            });
            ...
            // Poids facturé d'une ligne = max(poids réel, poids volume L×l×H cm ÷ 5000).
            ...
            const openClientModal = (target) => {
                ...
                if (target === 'exp') {
                    ...
                    if (_french) {
                        nextTick(() => Autocomplete.initAddress('faClientAdresse', 'faClientAdresseSuggestions'));
                    }
                }
            };
            ...
            const submitInvoice = async () => {
                ...
                batch.set(transRef, {
                    ...
                    departureAgency: activeAgency, // agence de DÉPART pour l'app Clients
    ...
    this.vueApp.mount('#vue-factureaerien-app');
}
};